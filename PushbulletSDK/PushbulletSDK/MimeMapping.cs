        ...
        private sealed class MimeMappingDictionaryIntegrated : MimeMappingDictionaryBase
        {
            ...
            protected override void PopulateMappings()
            {
                ...
                try
                {
                    int result=0;
                    int mimeMapCount=0;
                    //for (int i = 0; i <= mimeMapCount - 1; i++)
                    for (int i = 0; i < mimeMapCount; i++)
                    {
                        ...
                        try
                        {
                            int cBstrFileExtension=0;
                            int cBstrMimeType=0;
                            string fileExtension = (cBstrFileExtension > 0) ? StringFromWCharPtr(bstrFileExtension, cBstrFileExtension) : null;
                            string mimeType = (cBstrMimeType > 0) ? StringFromWCharPtr(bstrMimeType, cBstrMimeType) : null;
